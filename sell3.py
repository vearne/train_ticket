...
import time
import threading
from share_queue import ShareQueue
    ...
    for i in range(1, MAX_CARRIAGE_NUM + 1):
        for row in range(1, MAX_ROW_NUM + 1):
            for col in ['A', 'B', 'C', 'D', 'F']:
                ...
                ticket_dict[key].put(Seg(i, str(row) + col, 1, 10))

    return ticket_dict


class SellWorker(threading.Thread):
    def __init__(self, share_queue, ticket_dict, sellout_dict):
        threading.Thread.__init__(self)
        self.share_queue = share_queue
        self.sellout_dict = sellout_dict
        self.ticket_dict = ticket_dict

    def run(self):
        request = self.share_queue.get_one()
        while request is not None:
            user = request['user']
            start = request['start']
            end = request['end']
            ticket = self.sell(self.ticket_dict, start, end, self.sellout_dict)
            print '#' * 100
            print line.strip()
            if ticket:
                print user, "bought", ticket
            print '*' * 100
            print 'qsize', self.share_queue.qsize()
            print '*' * 100
            self.print_residual(self.ticket_dict)
            request = self.share_queue.get_one()

    def sell(self, ticket_dict, start, end, sellout_dict):
        '''
        ...
        for x in site_start:
            for y in site_end:
                ticket = ticket_dict[(x, y)].get_one()
                if ticket:
                    # 判断一下这个座位的站点区间是否被完全用完了
                    # 如果没有还需要把新生成的余票间存回去
                    if start > x:
                        temp = Seg(ticket.carriage_no, ticket.seat_no, x, start)
                        ticket_dict[(x, start)].put(temp)
                    if y > end:
                        temp = Seg(ticket.carriage_no, ticket.seat_no, end, y)
                        ticket_dict[(end, y)].put(temp)

                    return Seg(ticket.carriage_no, ticket.seat_no, start, end)

        if ticket is None:
            # 说明比能够覆盖该站点区间的所有区段都没有票了
            for x in site_start:
                for y in site_end:
                    sellout_dict[(start, end)] = True
        return None

    def print_residual(self, ticket_dict):
        print '------------余票情况-------------'
        for key in ticket_dict:
            if ticket_dict[key].qsize() > 0:
                print key, ticket_dict[key].qsize()
...
if __name__ == '__main__':
    ...
    with open('req.csv') as fp:
        for line in fp:
            ...
            request = {}
            request['user'] = ll[0]
            request['start'] = int(ll[1])
            request['end'] = int(ll[2])
            share_queue.put(request)

    thread_list = []
    for i in range(10):
        worker = SellWorker(share_queue, ticket_dict, sellout_dict)
        worker.start()
